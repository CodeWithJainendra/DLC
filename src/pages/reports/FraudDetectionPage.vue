<template>
      ...
      <VaCardTitle>
        ...
        <div class="flex gap-2">
          <VaSelect
            ...
            clearable
          />
          <VaSelect v-model="typeFilter" :options="fraudTypes" placeholder="Filter by Type" class="w-48" clearable />
        </div>
      </VaCardTitle>

      <VaCardContent>
        <VaDataTable :items="fraudCases" :columns="columns" :loading="loading" striped hoverable>
          <template #cell(riskLevel)="{ rowData }">
            <VaBadge :text="rowData.riskLevel" :color="getRiskColor(rowData.riskLevel)" />
          </template>

          <template #cell(fraudType)="{ rowData }">
            <VaBadge :text="rowData.fraudType" color="warning" outline />
          </template>
  ...
  <!-- Case Details Modal -->
  <VaModal v-model="showDetailsModal" title="Fraud Case Details" size="large" hide-default-actions>
    <div v-if="selectedCase" class="flex flex-col gap-4">
        ...
        <div>
          ...
          <div class="space-y-2">
            ...
            <p>
              ...
              <VaBadge :text="selectedCase.riskLevel" :color="getRiskColor(selectedCase.riskLevel)" />
            </p>
      ...
      <div class="flex justify-end gap-2 pt-4 border-t">
        ...
        <VaButton color="danger" @click="escalateCase(selectedCase)">Escalate</VaButton>
        <VaButton preset="primary" @click="markAsResolved(selectedCase)">Mark as Resolved</VaButton>
      </div>
    </div>
  </VaModal>
</template>
...
const fraudCases = ref<FraudCase[]>([
  {
    ...
    aiAnalysis:
      'The submitted photo matches with another pensioner profile (ID: UP2024005678) with 95% similarity. This indicates potential identity fraud or document reuse.',
  },
  {
    id: '2',
    ...
    aiAnalysis:
      'Multiple verification attempts from different geographical locations within a short time frame. This pattern suggests potential fraudulent activity.',
  },
])
