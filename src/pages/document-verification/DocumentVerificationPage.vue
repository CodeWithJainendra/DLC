<template>
      ...
      <VaCardTitle>
        ...
        <div class="flex gap-2">
          <VaSelect
            ...
            clearable
          />
          <VaSelect v-model="typeFilter" :options="documentTypes" placeholder="Filter by Type" class="w-48" clearable />
        </div>
      </VaCardTitle>

      <VaCardContent>
        <VaDataTable :items="documents" :columns="columns" :loading="loading" striped hoverable>
  ...
  <VaModal v-model="showViewerModal" title="Document Viewer" size="large" hide-default-actions>
    <div v-if="selectedDocument" class="flex flex-col gap-4">
        ...
        <div>
          ...
          <div class="space-y-2">
            <p>
              ...
              <VaBadge :text="selectedDocument.status" :color="getStatusColor(selectedDocument.status)" />
            </p>
            <p v-if="selectedDocument.verifiedBy"><strong>Verified By:</strong> {{ selectedDocument.verifiedBy }}</p>
            <p v-if="selectedDocument.verificationDate">
      ...
      <div v-if="selectedDocument.status === 'Pending'" class="border-t pt-4">
        ...
        <div class="flex justify-end gap-2 mt-4">
          ...
          <VaButton color="danger" @click="rejectDocument(selectedDocument)"> Reject Document </VaButton>
          <VaButton preset="primary" @click="approveDocument(selectedDocument)"> Approve Document </VaButton>
        </div>
      </div>

      <div v-else class="flex justify-end gap-2 pt-4 border-t">
        <VaButton @click="showViewerModal = false">Close</VaButton>
        <VaButton preset="secondary" @click="downloadDocument(selectedDocument)">Download</VaButton>
      </div>
    </div>
  </VaModal>
</template>
