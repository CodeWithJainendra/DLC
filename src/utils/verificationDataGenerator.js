...
const bankNames = [
  ...
  "Union Bank of India", "Bank of India", "Central Bank of India", "Indian Overseas Bank",
  "UCO Bank", "Bank of Maharashtra", "HDFC Bank", "ICICI Bank", "Axis Bank", "Kotak Mahindra Bank"
];
...
const generateCoordinatesForDistrict = (district, stateName) => {
  ...
  const matchingCity = indianCities.find(city =>
...
const getStateBasedFallbackCity = (stateName) => {
  const stateToCity = {
    ...
    "Andhra Pradesh": { name: 'Hyderabad', lat: 17.3850, lng: 78.4867 }
  };
...
export const generateVerificationData = () => {
  ...
  return {
    ...
    total_locations: states.reduce((sum, state) =>
      sum + state.districts.reduce((distSum, dist) => distSum + dist.locations.length, 0), 0
    )
  };
};
